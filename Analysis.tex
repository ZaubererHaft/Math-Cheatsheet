...
\begin{satz}[Taylor'sche Formel]
	Sei $\epsilon > 0, \f{(a-\epsilon, a +\epsilon)}{\realset}$ n-mal stetig differenzierbar. $f(x+h) = \sum_{k = 0}^n \frac{f^{(k)}(x)}{k!} \cdot h^k + o(h^n)$ für $h \rightarrow 0$. $T_{n}f(x; h) = \sum_{k = 0}^n \frac{f^{(k)}(x)}{k!} \cdot h^k$ heißt Taylorpolynom von $f$ in $x$.
\end{satz}

\begin{satz}[Taylor'sche Formel mit Restglied] 
	Eine weitere Darstellung der Taylor'schen Formel ist:
	$T_{n}f(x; h) = \sum_{k = 0}^n \frac{f^{(k)}(h)}{k!} \cdot (x \pm h)^k + \frac{(x \pm h)^{(n + 1)}}{(n + 1)!} \cdot f^{n + 1}(\xi) $
\end{satz}
