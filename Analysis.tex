...
\begin{definition}[Differentiationsregeln]
	content...
\end{definition}
...
\begin{definition}[Totale Differenzierbarkeit]
	Sei $U \subseteq \realset^n, \function{f}{U}{\realset^m}$. $f$ heißt in $x \in U $ total differenzierbar, falls es eine lineare Abbildung $\function{A}{\realset^n}{\realset^m} : h \mapsto A(h) := Ah, h = (h_1, ..., h_m)^T$ gibt, sodass gilt: 
	
	\begin{align*}
		\lim_{h \rightarrow 0} \frac{f(x + h) - f(x) - A \cdot h}{\norm{h}} = 0
	\end{align*}
	
	Diese Abbildung kann man als Matrix beschreiben $A \in \realset^{n \times m}$. Es gilt: $f$ ist differenzierbar $\leftrightarrow$ alle Komponenten sind in $x$ differenzierbar.
	Die Matrix $A$ heißt auch Jacobi Matrix. Man schreibt:
	$A = Jf(x) = (\frac{\partial f_i}{\partial x_j})_{1 \leq i \leq n, 1 \leq j \leq m} \in \realset^{n \times m}$
	
	Oder alternativ $f'(x) = h$ analog dem eindimensionalen Fall und nennen $f'(h)$ totale Ableitung. 		
\end{definition}
...
\begin{definition}[Totales/Vollständiges Differential]
	...
	\begin{align*}
	d f(\vec{x}) = \sum_{i = 1}^{n} \frac{\partial f (\vec{x}) }{\partial x_i} dx_i
	\end{align*}
...
\begin{satz}[Gradient und totale Ableitung]
	Sei $\f{\realset^n}{\realset}$ in $a$ total differenzierbar, d.h. $f'(a) = (c_1,...,c_n)$. Dann ist $f$ partiell differenzierbar und es gilt: $\partialf{f}{x_k}(a) = c_k$ bzw. $\nabla f (a)^T = f'(a) $
\end{satz}


\begin{satz}[Eigenschaften des Gradienten]
...
\begin{satz}[Kettenregel des Gradienten]
	\begin{align*}
		\nabla_y f(g(y)) \overset{x := g(y)}{=} (Jg)^T(y) \nabla_x f(x)
	\end{align*}
\end{satz}
...
{f(x,y)};
\end{axis}
\draw (aux1) -- ++ (-1,1) node[above,align=center]{Steigung in $x$ Richtung\\
...
{f(x,y)};
\end{axis}
\draw (aux2) -- ++ (0.3,1) node[above,align=center]{Steigung in $y$ Richtung\\
...
\section{Anwendung multivariabler Differentiation}
...
\begin{definition}[Quasilineare PDGL]
	content...
\end{definition}

\begin{definition}[Klassifikation von PDGL]
	content...
\end{definition}

\begin{definition}[Wohlgestellte PDGL]
	content...
\end{definition}
...
\begin{satz}[Lösungsverfahren Methode der Charaktersitiken]
	content...
\end{satz}
